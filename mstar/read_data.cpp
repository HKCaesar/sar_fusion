...
unsigned char* read_mstar(const char* MSTARname, const char* JPEGname, int HDRflag, int ENHANCEflag, int VERBOSEflag, int qfactor)
{
    ...
    // JPEGname=NULL;          unsigned   /* Ptr to output JPEG filename...   */
    //  HDRflag=FALSE;             /* If TRUE, dump Phoenix header..   */
    //  ENHANCEflag=FALSE;         /* If TRUE, apply enhancement..     */
    //  VERBOSEflag=FALSE;         /* If TRUE, keep user informed...   */
    //  qfactor                   compression factor  default 75
    ...
    FILE *MSTARphase=NULL;              /* Ptr to input MSTAR phase file          */
 ...
 ****************************************************/
    ...
    {
        // read data from tptr into pointer phlen
 ...
 ******************************************************/
    ...
    {
        case MSTAR_CHIP:
            bytesPerImage = 2*nchunks * sizeof(float);
            ...
            switch (byteorder)
            {
                ...
                case MSB_FIRST: /* Big-endian..no swap */
                    ...
                    numgot = fread(CHIPdata, sizeof(float), 2*nchunks, MSTARfp);
                    break;
            }
